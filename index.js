...
async function configCheck() {
	...
	//==== Check Avaliable Storage System
	const validStorageSystems = ['cloudreve'];
	if (!validStorageSystems.includes(UploadStorageSystem)) {
		logger.error(`Invalid UploadStorageSystem: ${UploadStorageSystem}. Valid options are: ${validStorageSystems.join(', ')}`);
		errorCount++;
	}

	if (errorCount > 0) {
		logger.line();
		logger.error(`Config check failed with ${errorCount} error(s). Please fix the issues and restart the bot.`);
		return false;
	} else {
...
async function init() {
	...
	console.log(color.yellow('[Loading Interactions...]                  [Loading Events...]'));
